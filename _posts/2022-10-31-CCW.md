...
$$\overrightarrow{p_1 p_2} \times \overrightarrow{p_2 p_3} = v_{1x} \times v_{y2} - v_{2x} \times v_{1y}$$
...
이런 모순을 해결하고자 부호규약이 정해져있다. 회전력의 계산 결과가 +라면 두 벡터의 곱의 방향을 후에 보정해주는 식으로 식이 구성된다. 좌표계에서 힘의 방향을 임의로 정하면서 생기는 혼란을 후보정하는 것이다.
...
print(2 * (cross > 0) - 1 if cross else O)
