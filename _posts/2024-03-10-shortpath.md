...
{% include rate.html image_path="/assets/images/rate/R5.svg" url="https://www.acmicpc.net/problem/18190" discription="4212 최단 비행 경로"%}
...
3차원 기하학과 데이크스트라가 적절히 섞여있는 문제입니다. 구사과 님의 코드를 해석하고 공부해서 구면좌표계를 다루는 방법을 터득하고, 예전에 풀었던 원반 위의 데이크스트라 문제를 응용해서 구면 위 원반 위에서의 데이크스트라를 구현했습니다.

## 기하학

구조체 `Pos3D`를 만들어 3차원 기하학을 구현합니다. 연산용 함수를 만들어줄 수도 있지만, 연산자를 오버로딩해서 쓰는 게 더 간결해지므로 연산자들을 오버로딩하겠습니다. 단위 벡터, 벡터의 크기를 구하는 함수도 만들어줍시다. 그리고 구면좌표계와 직교좌표계 변환 함수를 만들어줍니다.
...
두 원의 중심과 구의 중심 사이의 거리는 1이라고 하겠습니다. 이제부터 설명하는 모든 벡터는 원점 기준 1의 거리로 떨어져있다고 생각하면 됩니다. </br>두 원의 중점을 구해줍니다. 중점과 구의 원점이 같은 경우 교점은 없거나 무한하므로 구하지 않습니다. </br>두 원들의 중심의 중심을 `mid`라고 하겠습니다. `mid`의 크기가 의미하는 것은 두 원 중 하나의 중심으로부터 `mid`까지 이동했을 때의 호의 각도의 `cos`값이 됩니다.

![spath2](/assets/images/2024-03-10-sphere-dijk/spath02.jpg)

비행기의 허용 이동 거리를 호로 표현했을 때 호의 각도 Θ의 `cos`값과 비교해주고 -1 에서 1 사이에 있지 않다면 허용 거리를 초과해서 이동해야한다는 뜻이 되므로 교점이 존재하지 않습니다. 나머지는 피타고라스의 공식을 응용해 두 교점을 구합니다. 교점이 하나만 발생하는 경우가 발생해서 조금 특수하게 구했습니다.</br>말로만 설명하딘 어려울 것 같아 그림을 그렸습니다만 그래도 이해가 어렵다면 직접 그려보시길 바랍니다.
</br>
스위핑을 하기 위해서는 두 원을 이은 경로 사이에 있는 모든 교점을 구해줄 필요가 있습니다. 두 원의 외적을 구해서 평면의 법선 벡터를 구해줍니다. 법선 벡터를 목표 원의 중심 벡터에 사영하고, 그 크기를 법선 벡터에 적용해 빼주면 목표 원으로부터 평명까지의 수선의 발을 내릴 수 있게 됩니다. 이후는 위와 같은 과정으로 교점을 구해줍시다.
...
이 경로가 존재하는 평면에서 생기는 모든 교점을 찾아줍니다. 시작점의 각도를 0, 도착점의 각도는 ang, 그리고 다른 모든 교점들의 각도는 두 점이 만든 각도 기준으로 계산해줍니다. 원의 시작에서 생기는 교점에는 -1, 끝에서 생기는 교점에는 1을 부여하고, 시작점과 도착점에는 0을 부여해서 컨테이너에 넣고 정렬해줍니다. 하나씩 하나씩 꺼내면서 시작점이 원 안에 들어있는지, 도중에 모든 원을 벗어나는지, 도착점까지 도착할 수 있는지를 보고, 가능하다면 1, 불가능하다면 0을 반환합니다.
...
쿼리를 받고 두 점 사이의 최단 경로를 구하는 일만 남았습니다.

# 데이크스트라

두 공항 사이의 최단 경로를 구하기 위해서 모든 점들에 대해 최단 경로를 구해줘야 합니다. 그 전에 생각해보면, 원과 원에 의해 생기는 교점들은 최종 쿼리를 수행할 때는 고려할 필요가 없습니다. 허용 거리 안에서 도달할 수 있는 '공항'만 연료를 충전할 수 있기 때문입니다. 그러니 이렇게 해보겠습니다.

1. 일단 N개의 공항에 대해 다른 모든 점으로 가는 최단 경로를 N번 데이크스트라를 시행해서 구한다.
2. 데이크스트라 결과로 구한 비용은 출발 공항에서 각 도착 공항들에 대한 비용 그래프이므로, 이를 N번 만들어 최종본을 만든다.
3. 구해낸 최종 지도와 한계 허용 거리를 가지고 쿼리를 수행한다.
...
풀이를 온전히 제가 생각해낸 건 아니고, 구사과님의 코드를 참고해서 구현 방법을 연구했고 별도의 기하학 공부를 병행해서 풀었습니다. </br>스위핑과 데이크스트라를 직접 개선해서 참고했던 원본 코드의 약 11배의 성능 개선을 이뤄냈으니 만족합니다.
